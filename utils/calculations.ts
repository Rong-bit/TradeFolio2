...
const fetchSingleStockPrice = async (symbol: string): Promise<PriceData | null> => {
  try {
    ...
    // 如果 API 沒有提供，則自己計算
    let change: number | undefined = meta.regularMarketChange;
    let changePercent: number | undefined = meta.regularMarketChangePercent;
    ...
    return {
      price: regularMarketPrice,
      change: change !== undefined && change !== null ? change : 0,
      changePercent: changePercent !== undefined && changePercent !== null ? changePercent : 0,
    };
  } catch (error) {
