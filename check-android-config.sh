#!/bin/bash
# Android 配置檢查腳本

echo "========================================"
echo "Android 配置檢查"
echo "========================================"
echo ""

# 檢查 gradle.properties
echo "1. 檢查 gradle.properties 簽名配置..."
if grep -q "RELEASE_STORE_FILE" gradle.properties && ! grep -q "^#.*RELEASE_STORE_FILE" gradle.properties; then
    echo "   ✅ 簽名配置已設置"
else
    echo "   ⚠️  簽名配置未設置，請編輯 gradle.properties"
fi

# 檢查 build.gradle 中的網址配置
echo ""
echo "2. 檢查 build.gradle 網址配置..."
if grep -q "hostName: 'rong-bit.github.io'" app/build.gradle; then
    if grep -q "launchUrl: '/TradeFolio/'" app/build.gradle; then
        echo "   ✅ 網址配置正確: https://rong-bit.github.io/TradeFolio/"
    else
        echo "   ⚠️  網址配置可能不正確，請檢查 launchUrl"
    fi
else
    echo "   ⚠️  請檢查網址配置"
fi

# 檢查版本號
echo ""
echo "3. 檢查版本號..."
VERSION_CODE=$(grep -A 1 "versionCode" app/build.gradle | grep -o "[0-9]*" | head -1)
VERSION_NAME=$(grep -A 1 "versionName" app/build.gradle | grep -o '"[^"]*"' | head -1)
echo "   Version Code: $VERSION_CODE"
echo "   Version Name: $VERSION_NAME"

# 檢查 AAB 文件是否存在
echo ""
echo "4. 檢查構建文件..."
if [ -f "app/build/outputs/bundle/release/app-release.aab" ]; then
    AAB_SIZE=$(du -h app/build/outputs/bundle/release/app-release.aab | cut -f1)
    echo "   ✅ AAB 文件已存在 (大小: $AAB_SIZE)"
else
    echo "   ⚠️  AAB 文件不存在，請先執行構建"
fi

# 檢查 Digital Asset Links
echo ""
echo "5. 檢查 Digital Asset Links..."
echo "   ⚠️  請確認以下文件已部署到 GitHub Pages 域名根目錄:"
echo "   https://rong-bit.github.io/.well-known/assetlinks.json"
echo "   注意: 即使網站在 /TradeFolio/ 子目錄，此文件也必須放在域名根目錄"
echo "   使用範例文件: .well-known/assetlinks.json.example"

echo ""
echo "========================================"
echo "檢查完成"
echo "========================================"

